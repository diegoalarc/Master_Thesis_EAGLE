...
Field_Carmen_nocorr$VH_median <- NULL
...
# Variable importance
varimp_data_rf <- data.frame(feature = row.names(varImp(model_rf)$importance)[1:11],
                             importance = varImp(model_rf)$importance[1:11, 1])
...
ggplot(data = varimp_data_rf, 
  ...
  labs(title = 'RF - Variable Importance S2') + 
...
# Variable importance
varimp_data_crf <- data.frame(feature = row.names(varImp(model_crf)$importance)[1:11],
                              importance = varImp(model_crf)$importance[1:11, 1])
...
ggplot(data = varimp_data_crf, 
  ...
  #theme(axis.text.x = element_text(angle = 45, hjust = 1, colour = "black")) +
  theme(axis.text.x=element_text(colour="black", angle = 30, size = 10)) +
  labs(title = 'CIF - Variable Importance S2') + 
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(legend.position = "none")
...
# Variable importance
varimp_data_gau <- data.frame(feature = row.names(varImp(model_gau)$importance)[1:11],
                              importance = varImp(model_gau)$importance[1:11, 1])
...
ggplot(data = varimp_data_gau, 
  ...
  #theme(axis.text.x = element_text(angle = 45, hjust = 1, colour = "black")) +
  theme(axis.text.x=element_text(colour="black", angle = 30, size = 10)) +
  labs(title = 'GPR - Variable Importance S2') + 
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(legend.position = "none")
...
ggplot(values_crf, aes(x = Observed, y = Predicted)) +
  ...
  #xlim(0,80000) + ylim(0,80000) +
  xlim(30000,60000) + ylim(30000,60000) +
  labs(title = "Conditional inference forests - Comparison S2",
       x = "Observed", y = "Predicted ") +
  theme_classic()
...
ggplot(values_gau, aes(x = Observed, y = Predicted)) +
  ...
  #xlim(0,80000) + ylim(0,80000) +
  xlim(30000,60000) + ylim(30000,60000) +
  labs(title = "Gaussian Process Regression - Comparison S2",
       x = "Observed", y = "Predicted ") +
  theme_classic()
