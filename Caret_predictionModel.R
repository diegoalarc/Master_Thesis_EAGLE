...
# variable importance
gbmImp_rf <- varImp(model_rf, useModel = TRUE)
gbmImp_rf
...
plot(gbmImp_rf, top = 28, main = "Conditional Random Forests - Variable Importance plot")

dev.off()

# variable importance
# to check the metric use list_metrics()
pfun <- function(object, newdata) predict(object, newdata = newdata)
gbmImp_rf <- vip(model_rf, method = "permute", train = Field_Carmen, target = "Kg_He", 
                 nsim = 5, metric = "rsquared", pred_wrapper = pfun, geom = "boxplot",
...
nrow(varImp(model_rf)$importance) #61 variables extracted
...
varimps_rf$label[grepl("SLC", varimps_rf$variable)] <- 'Remote Sensing'
...
varimps_crf$label[grepl("SLC", varimps_crf$variable)] <- 'Remote Sensing'
