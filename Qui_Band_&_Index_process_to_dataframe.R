...
roi <- readOGR(dsn=file.path('./ROI/Quimetal.shp'))
...
for (i in 1:length(stat_type)){
  ...
  rasdir <- file.path(paste0('./Satellite_data/Quimetal/'))
  ...
  print(paste0('ROI total number: ', length(roi)))
  ...
  for (x in 1:length(roi)){
    ...
    for (i in 1:nlayers(fllst_stack)){
      ...
      my_df[i,2] <- as.numeric(exact_extract(fllst_stack[[i]], roi_1, 'mean', progress = TRUE))
  ...
  # Since the 4 polygons are considered together a single field, the information
  # must be joined last
  df1 <- mutate_all(band_big_data[-21], function(x) as.numeric(as.character(x)))
  df2 <- as.data.frame.list(colMeans(df1))
  df2$Field <- c('THOMPSON LORETO')
  
  # Save dataframe as .CSV
  write.csv(df2, 
            paste0('./Original_data/Values_bands/', 
                   farm_name, '_Band_', type_now, '_2019.csv'), 
            row.names = F, quote = F)
  
  # Print the document
  print(paste0('Document created: ', farm_name,'_Band_', type_now, '_2019.csv'))
}
